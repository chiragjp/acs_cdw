...
safe_div <- function(num, denom) {
  if_else(denom == 0 | is.na(denom), NA_real_, num / denom)
}
...
## get population data
get_acs_table_tracts_for_state_with_geom <- function(state, table_name, year, geometry=TRUE, max_retries=3) {
  for (attempt in 1:max_retries) {
    tab <- tryCatch({
      get_acs(geotype, year=year, key=api_key, state = state, table = table_name, geometry = geometry)
    }, error = function(e) {
      cat(sprintf("  Attempt %d/%d failed for %s: %s\n", attempt, max_retries, table_name, e$message))
...
un <- un %>% mutate(d_unemployed_percentage=safe_div(estimate_B23025_005, estimate_B23025_001))
...
poverty <- poverty %>% mutate(d_below_poverty_line_percentage=safe_div(estimate_B17020_002, estimate_B17020_001))
# percent who are older (>60) | under poverty
poverty <- poverty %>% mutate(d_percent_older_under_poverty=safe_div(estimate_B17020_007+estimate_B17020_008+estimate_B17020_009, estimate_B17020_002))
...
education <- education %>% mutate(d_education_less_hs_male_over_25_percentage =
                                    safe_div(estimate_B15001_012 + estimate_B15001_013 + estimate_B15001_020 + estimate_B15001_021 + estimate_B15001_028 + estimate_B15001_029 +  estimate_B15001_036 + estimate_B15001_037, d_total_male_over_age_25))
education <- education %>% mutate(d_education_less_hs_female_over_25_percentage =
                                    safe_div(estimate_B15001_053 + estimate_B15001_054 + estimate_B15001_061 + estimate_B15001_062 + estimate_B15001_069 + estimate_B15001_070 + estimate_B15001_077 + estimate_B15001_078, d_total_female_over_age_25))
education <- education %>% mutate(d_education_greater_four_year_college_male_over_25_percentage =
                                    safe_div(estimate_B15001_017 + estimate_B15001_018 + estimate_B15001_025 + estimate_B15001_026 + estimate_B15001_033 + estimate_B15001_034 + estimate_B15001_041 + estimate_B15001_042, d_total_male_over_age_25))
education <- education %>% mutate(d_education_greater_four_year_college_female_over_25_percentage =
                                    safe_div(estimate_B15001_058 + estimate_B15001_059 + estimate_B15001_066 + estimate_B15001_067 + estimate_B15001_074 + estimate_B15001_075 + estimate_B15001_082 + estimate_B15001_083, d_total_female_over_age_25))

education <- education %>% mutate(d_education_less_hs_over_25_percentage =
                                    safe_div(estimate_B15001_012 + estimate_B15001_013 + estimate_B15001_020 + estimate_B15001_021 + estimate_B15001_028 + estimate_B15001_029 +  estimate_B15001_036 + estimate_B15001_037
...
education <- education %>% mutate(d_education_greater_four_year_college_over_25_percentage =
                                    safe_div(estimate_B15001_017 + estimate_B15001_018 + estimate_B15001_025 + estimate_B15001_026 + estimate_B15001_033 + estimate_B15001_034 + estimate_B15001_041 + estimate_B15001_042 +
...
crowded <- crowded %>% mutate(d_households_containing_greater_1_per_room = safe_div(estimate_B25014_007 + estimate_B25014_013, estimate_B25014_001))
...
sex_by_age <- sex_by_age %>% mutate(d_male_over_65_percent= safe_div(estimate_B01001_020+estimate_B01001_021+estimate_B01001_022+estimate_B01001_023 + estimate_B01001_024 + estimate_B01001_025, d_male_total))
sex_by_age <- sex_by_age %>% mutate(d_male_over_75_percent= safe_div(estimate_B01001_023 + estimate_B01001_024 + estimate_B01001_025, d_male_total))
sex_by_age <- sex_by_age %>% mutate(d_male_over_85_percent= safe_div(estimate_B01001_025, d_male_total))

sex_by_age <- sex_by_age %>% mutate(d_female_over_65_percent= safe_div(estimate_B01001_044+estimate_B01001_045+estimate_B01001_046+estimate_B01001_047 + estimate_B01001_048 + estimate_B01001_049, d_female_total))
sex_by_age <- sex_by_age %>% mutate(d_female_over_75_percent= safe_div(estimate_B01001_047 + estimate_B01001_048 + estimate_B01001_049, d_female_total))
sex_by_age <- sex_by_age %>% mutate(d_female_over_85_percent= safe_div(estimate_B01001_049, d_female_total))
...
insurance <- insurance %>% mutate(d_no_health_insurance_percentage= safe_div(estimate_B27011_007 + estimate_B27011_012 + estimate_B27011_017, estimate_B27011_001))
insurance <- insurance %>% mutate(d_public_health_insurance_percentage= safe_div(estimate_B27011_006 + estimate_B27011_011 + estimate_B27011_016, estimate_B27011_001))
insurance <- insurance %>% mutate(d_no_insurance_unemployed_percentage= safe_div(estimate_B27011_012, estimate_B27011_008))
...
sex_by_age_dis <- sex_by_age_dis %>% mutate(d_disability_male_over65_percentage = safe_div(estimate_B18101_016 + estimate_B18101_019, estimate_B18101_015 + estimate_B18101_018))
sex_by_age_dis <- sex_by_age_dis %>% mutate(d_disability_female_over65_percentage = safe_div(estimate_B18101_035 + estimate_B18101_038, estimate_B18101_034 + estimate_B18101_037))
...
sex_by_age_hearing <- sex_by_age_hearing %>% mutate(d_hearing_male_over65_percentage = safe_div(estimate_B18102_016 + estimate_B18102_019, estimate_B18102_015 + estimate_B18102_018))
sex_by_age_hearing <- sex_by_age_hearing %>% mutate(d_hearing_female_over65_percentage = safe_div(estimate_B18102_035 + estimate_B18102_038, estimate_B18102_034 + estimate_B18102_037))
...
sex_by_age_cog <- sex_by_age_cog %>% mutate(d_cognitive_male_over65_percentage = safe_div(estimate_B18104_013 + estimate_B18104_016, estimate_B18104_012 + estimate_B18104_015))
sex_by_age_cog <- sex_by_age_cog %>% mutate(d_cognitive_female_over65_percentage = safe_div(estimate_B18104_029 + estimate_B18104_032, estimate_B18104_028 + estimate_B18104_031))
...
sex_by_age_amb <- sex_by_age_amb %>% mutate(d_ambulatory_male_over65_percentage = safe_div(estimate_B18105_013 + estimate_B18105_016, estimate_B18105_012 + estimate_B18105_015))
sex_by_age_amb <- sex_by_age_amb %>% mutate(d_ambulatory_female_over65_percentage = safe_div(estimate_B18105_029 + estimate_B18105_032, estimate_B18105_028 + estimate_B18105_031))
...
sex_by_age_sc <- sex_by_age_sc %>% mutate(d_selfcare_male_over65_percentage = safe_div(estimate_B18106_013 + estimate_B18106_016, estimate_B18106_012 + estimate_B18106_015))
sex_by_age_sc <- sex_by_age_sc %>% mutate(d_selfcare_female_over65_percentage = safe_div(estimate_B18106_029 + estimate_B18106_032, estimate_B18106_028 + estimate_B18106_031))
...
sex_by_age_ild <- sex_by_age_ild %>% mutate(d_inddifficulty_male_over65_percentage = safe_div(estimate_B18107_010 + estimate_B18107_013, estimate_B18107_009 + estimate_B18107_012))
sex_by_age_ild <- sex_by_age_ild %>% mutate(d_inddifficulty_female_over65_percentage = safe_div(estimate_B18107_023 + estimate_B18107_026, estimate_B18107_022 + estimate_B18107_025))
...
race <- race %>% mutate(d_white_percentage = safe_div(estimate_B02001_002, estimate_B02001_001),
                        d_black_percentage = safe_div(estimate_B02001_003, estimate_B02001_001),
                        d_native_percentage = safe_div(estimate_B02001_004, estimate_B02001_001),
                        d_asian_percentage = safe_div(estimate_B02001_005, estimate_B02001_001),
                        d_hawaii_pi_percentage = safe_div(estimate_B02001_006, estimate_B02001_001),
                        d_other_race_percentage = safe_div(estimate_B02001_007, estimate_B02001_001),
                        d_two_more_race_percentage = safe_div(estimate_B02001_008, estimate_B02001_001),
                        d_two_more_race_incl_other_percentage = safe_div(estimate_B02001_009, estimate_B02001_001))
...
heating <- heating %>% mutate(d_energy_gas_percentage=safe_div(estimate_B25040_002, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_bottledgas_percentage=safe_div(estimate_B25040_003, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_electricity_percentage=safe_div(estimate_B25040_004, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_fueloil_percentage=safe_div(estimate_B25040_005, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_coal_percentage=safe_div(estimate_B25040_006, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_wood_percentage=safe_div(estimate_B25040_007, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_solar_percentage=safe_div(estimate_B25040_008, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_otherfuel_percentage=safe_div(estimate_B25040_009, estimate_B25040_001))
heating <- heating %>% mutate(d_energy_nofuel_percentage=safe_div(estimate_B25040_010, estimate_B25040_001))
